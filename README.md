...
这是有意的取舍：代价是 key 存在浏览器且出现在请求体中（DevTools 可见），换来的是任何人都能用自己的额度体验，站点方无需承担成本或保管他人密钥。若要改回服务端持有密钥，只需让 [provider.ts](src/lib/ai/provider.ts) 读环境变量而非请求参数。

插图走免费的 Pollinations，不需要 key；未配置模型时插图静默降级为无图，不阻塞剧情。
